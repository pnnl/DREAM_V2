...
public class H5File extends FileFormat {
    ...
    public FileFormat createInstance(String filename, int access) throws Exception {
        log.trace("createInstance start");
    ...
    public HObject get(String path) throws Exception {
        ...
        log.trace("H5File:get path:{}", path);
        ...
        log.trace("H5File:get fid_before_open with:{}", fid);
        fid = open(false);
        if (fid < 0) {
        ...
        try {
            ...
            log.trace("H5File:get H5Oopen:{}", oid);

            if (oid >= 0) {
                info = H5.H5Oget_info(oid);
                log.trace("H5File:get H5Oget_info-type:{}", info.type);
                objType = info.type;
                if (objType == HDF5Constants.H5O_TYPE_DATASET) {
                    int did = -1;
                    try {
                        log.trace("H5File:get H5O_TYPE_DATASET:{}-{}", name, pPath);
                    ...
                    int gid = -1;
                    try {
                        log.trace("H5File:get H5O_TYPE_GROUP:{}-{}", name, pPath);
    ...
    private int open(boolean loadFullHierarchy, int plist) throws Exception {
        ...
        try {
            ...
            log.trace("open: open file exception", ex);
            try {
                log.trace("open: open file read only");
                fid = H5.H5Fopen(fullFileName, HDF5Constants.H5F_ACC_RDONLY, HDF5Constants.H5P_DEFAULT);
                isReadOnly = true;
            }
            catch (Exception ex2) {
                log.trace("open: open file read only exception", ex2);
                ...
                if (idx > 0) {
                    ...
                    int pid = -1;
                    try {
                        log.trace("open: open file family");
                        pid = H5.H5Pcreate(HDF5Constants.H5P_FILE_ACCESS);
     ...
     *            If there is a failure.
     */
    private TreeNode copyGroup(H5Group srcGroup, H5Group dstGroup, String dstName) throws Exception {
        H5Group group = null;
        DefaultMutableTreeNode newNode = null;
